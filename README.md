...
本项目本身只对接企业微信自建应用。**个人微信的桥接依赖企业微信「微信插件」能力**：

1. 企业微信自建应用开启 API 接收消息（回调 URL 指向本插件）
2. 管理后台进入「我的企业 → 微信插件」，让个人微信扫码关联企业账号
3. 个人微信在微信插件里打开企业微信应用，消息会进入企业微信回调
4. 插件将消息转给 OpenClaw，回复再通过企业微信 API 返回到个人微信
